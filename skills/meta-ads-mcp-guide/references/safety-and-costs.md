...
- `delivery=inline` on a video is measured in megabytes; only use it for a client whose model ingests video.
- The server caps every response, and says so in a warning when it had to cut something.
